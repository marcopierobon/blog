...
The main difference in this regard, is that it is available via a MultiLangDaemon ([MultiLangeDaemon Code][8]) app that allows call from other languages. In order to make this possible, the Java process need to be running side by side with the code in the other language ([MultiLangDaemon on AWS docs][9]).
...
- checkpointing processed records: the status of the consumers (meaning which records have been already read and which have not), are automatically persisted in an underlying DynamoDB table by the KCL
...
- the number of events per second using the same `PartitionKey` need to be considered: each shard can ingest up to 1MB/s per second or 1000 records per second (whichever come first). So, in case of having spikes of records with the same `PartitionKey` this will likely result in one or more hot shards and ultimately in throttling.


## On batching

Performing one PutRecords operation for N records has the following advantages over performing N PutRecords operations:
- the latency cost is payed once instead of N times
